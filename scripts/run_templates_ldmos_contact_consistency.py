"""Paired R7 high-voltage controls for one-time finite-contact algebraic consistency."""
import argparse
import copy
import json
import os
...
def analyze(root):
    ...
    expected = {(g, i, r, v) for g, i in CASES for r in range(s['repeats']) for v in ('baseline', 'contact')}
    ...
    for g, i in sorted(CASES):
        ...
        for repeat in range(s['repeats']):
            ...
            c, cc, cd = loaded[g, i, repeat, 'contact']
            expected_cfg = dict(bc, **{FLAG: True})
            if cc != expected_cfg: raise ValueError('Non-isolated configuration')
            event = cd['near_steady_contact_consistency']['events']
            if len(event) > 1: raise ValueError('Repeated contact')
            ...
            if event:
                ...
                raise ValueError('Inactive contact changed result')
            if event and not event[0]['accepted']:
                for key in ('history','state_interleaved','referenced_state_interleaved','residual','carrier_row_gate'):
                    if cd[key] != bd[key]: raise ValueError('Rejected repair changed fallback: '+key)
            ...
            pairs.append(dict(repeat=repeat, qualified=b['qualified'] and c['qualified'],
                ...
                contact={k:c[k] for k in ('newton_updates','attempts','trials','assemblies','factorizations','wall_seconds')},
                ...
                contact_floor=sum(h['scaled_l2_before']<1e-9 for h in cd['history']),
        ...
        findings.append(dict(vg=g,index=i,pairs=pairs,repeated_trajectory_exact=True,
            ...
            median_contact_seconds=statistics.median(p['contact']['wall_seconds'] for p in pairs)))
    result=dict(summary_sha256=sha(root/'summary.json'), findings=findings,
        scope='Frozen original seeds, identical inputs except opt-in contact repair. All attempts and rebuilds charged. Child process wall includes trace/output; not full-curve production timing.')
...
def main():
    ...
    s=dict(schema='vela.r7_local_contact.v1',status='running',repeats=args.repeats,
    ...
    for repeat in range(args.repeats):
        for case in cases:
            ...
            for variant in (('baseline','contact') if repeat%2==0 else ('contact','baseline')):
                cfg=copy.deepcopy(original)
                if variant=='contact': cfg[FLAG]=True
