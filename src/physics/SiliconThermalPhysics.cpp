...
SiliconThermalResult SiliconThermalPhysics::evaluate(const SiliconThermalState&s) const {
    ...
    const D psi=variable(s.potential_V,0),fn=variable(s.electronQf_V,1),fp=variable(s.holeQf_V,2),t=variable(s.temperature_K,3);
    const D vt=scalar(constants::kb/constants::q)*t,ratio=t/scalar(300.);
    const auto b=bands(p_,t);SiliconThermalResult r;r.bandgap_eV=b[0];r.Nc_m3=b[1];r.Nv_m3=b[2];
    const Real doping=s.donors_m3+s.acceptors_m3;
    ...
    if(p_.fermiBgnCorrection)delta+=fermiStatisticsBandgapCorrection(s.donors_m3,s.acceptors_m3,Nc300_,Nv300_,constants::Vt_300);
    ...
    r.electrons_m3=b[1]*fN;r.holes_m3=b[2]*fP;
    ...
    r.augerElectron_m6_per_s=polynomial(p_.augerElectron_m6_per_s,ratio)*(scalar(1.)+scalar(p_.augerEnhancement[0])*exp(scalar(0.)-r.electrons_m3/scalar(p_.augerReference_m3[0])));
    r.augerHole_m6_per_s=polynomial(p_.augerHole_m6_per_s,ratio)*(scalar(1.)+scalar(p_.augerEnhancement[1])*exp(scalar(0.)-r.holes_m3/scalar(p_.augerReference_m3[1])));
