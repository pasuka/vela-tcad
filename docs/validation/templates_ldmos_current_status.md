...
**R10 显式推荐配置**；R9 回退、导出器兼容默认 R7 及 C++ 通用默认不变。
...
此前接触一致性方案命名为 R9；本轮只在其上增加准备/高场复用，见
[R10 生产复现指南](templates_ldmos_production_reproduction.md#recommended-explicit-r10-preparation-reuse-profile)。
导出器兼容默认仍为 R7；选择 R10/R9 必须显式给定 profile。资格对应各自
...
| 生产 DC 电热入口集成 | 显式 `electrothermal_dc_sweep` 支持中性初始化、预偏置、四方程扫描、恢复及输出；R10 跨点复用和新进程真实恢复通过 | 普通 `dc_sweep` 仍为等温；R10 必须显式选择，R9 保留回退 |
| 整体性能与重复性 | R10 两轮全部非计时轨迹与 R9 一致，四组墙钟均减少，均通过原生 1.5 倍门限 | CPU 仍为原生 1.058–1.686 倍；更新 344/302、非 floor 343/297 不变，绝对耗时有波动 |
| 最终联合验收 | R10 完成前8点真实恢复、完整62点电学/热学/局部场及两轮逐组墙钟验收 | 资格随冻结程序和输入；仅准备/高场复用晋级，中性根缓存/标量 Newton/局部限幅未纳入 |

G3/D5/D4 已按各阶段冻结配置完成验收及共享路径复核；本次 R10 阶段未
