...
D0默认与原显式Halley的62对状态最大差为0，更新344/303、求根回退0/0。
D5最大电流误差1.592417%/1.625083%，D4为1.891761%/1.801547%，
原数值、曲线及D0热学门限均保持并通过。本轮实现、测试、复核脚本及验证记录一并纳入本地分支提交。

新增 [屏蔽求根完整曲线与重复计时](templates_ldmos_screening_full_2026-09-25.md)：
同一Codespace Release/UMFPACK单线程下，四种方法的8条D0完整资格曲线及
Halley/legacy三轮新配对共20条曲线、620点、10组原生联合验收、744对状态
检查全部通过。三轮双栅压墙钟均值319.921 → 298.497 s，合并下降6.70%，
逐轮5.68%–7.24%，六组栅压配对均更快；第三轮波动原样保留。
漏压Newton仍为344/303，试算472/424；装配数786/689 → 784/692，不能称为
完全相同装配工作量。求根回退为0，无新增失败恢复；原门限未变。
该轮摘要证据已回收并核验125个文件散列，当时默认仍为legacy；随后默认化
见上方新报告。该轮不认领三候选之间的稳定排名或异机性能。

新增 [Codespaces 屏蔽求根候选验证](templates_ldmos_screening_candidates_2026-09-24.md)：
本地main已合并（`3aabecf2`及合并测试补正`b80b9332`）；候选实现随上述默认化集成一并纳入本地分支提交。
GCC16 Release完整CTest 856/856、专项11例/20,436断言、8条D0短曲线/64点通过。
Newton/Halley/TOMS748一轮双栅压合计下降6.36%/7.18%/6.32%，
漏压Newton均为152/137，轨迹一致；最大保存状态差3.35e-12。
Halley标量三轮微基准约7倍；该轮未包含完整曲线重复计时/原生联合验收，
该阶段生产默认仍为legacy；没有在当时不可用的T470p上启动任务。

新增 [T470p 三项独立子成本补测](templates_ldmos_thermal_subcost_2026-09-24.md)：
Release/UMFPACK 1/1，8条D0完整曲线、4组原生联合验收和248对状态对照通过，
状态数值及Newton/推进/装配工作量不变，相关9例/13,038断言通过。
漏压屏蔽求根18.681/16.468 s，符号分析1.104/1.056 s；热矩阵独占原始计时
1.954/1.740 s含显著细粒度时钟偏差，不可直接认领全部为可省成本。
数值分解42.145/38.324 s另列；初始化单列。诊断默认关闭，没有实施上述优化。
批次与联合审核已结束，T470p相关进程退出；完整口径和证据见报告。
