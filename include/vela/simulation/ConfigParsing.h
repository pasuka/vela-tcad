...
inline void appendConfigFixedChargeSpec(
...
inline std::vector<RegionFixedChargeSpec> parseRegionFixedChargeSpecs(
    const nlohmann::json& cfg)
{
    ...
    if (cfg.contains("doping")) {
        for (const auto& entry : cfg.at("doping")) {
            ...
            detail::appendConfigFixedChargeSpec(
                specs,
                sourcesByRegion,
...
inline std::vector<InterfaceSheetChargeSpec> parseInterfaceSheetChargeSpecs(
    const nlohmann::json& cfg)
{
    ...
    for (const auto& entry : cfg.at("interfaces")) {
        ...
        if (entry.contains("trap_density_m2") &&
            ...
            throw std::runtime_error(
                "ConfigParsing: trap_occupancy must be in [0, 1] when trap_density_m2 is provided.");
        }
