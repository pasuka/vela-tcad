#!/usr/bin/env python3
"""Regression coverage for the VanOverstraeten A-scale full BV matrix runner."""
...
SCRIPT = REPO / "scripts" / "run_vanoverstraeten_A_scale_full_bv_matrix.py"


class VanOverstraetenAScaleFullBvMatrixTest(unittest.TestCase):
    def test_skip_run_summarizes_existing_a_scale_case_outputs(self) -> None:
        with tempfile.TemporaryDirectory(prefix="vela_vo_a_scale_full_bv_") as td:
            tmp = Path(td)
            out_dir = tmp / "matrix"
            ...
            cases = [
                ("BV-A1-default", "1", 1.0, "1"),
                ("BV-A2-scale-2", "2", 2.0, "1"),
                ("BV-A4-scale-4", "4", 5.0, "0"),
            ]
            ...
            out_csv = out_dir / "vanoverstraeten_A_scale_full_bv_matrix.csv"
            summary_md = out_dir / "vanoverstraeten_A_scale_full_bv_matrix_summary.md"
            ...
            self.assertIn("A_scale=2", text)
            self.assertIn("-20V", text)
